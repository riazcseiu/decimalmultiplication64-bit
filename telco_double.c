...
int main(int argc, char *argv[]) {
 ...
 // printf("new double telco");
  ...
  // Get any parameters
 // strcpy(filein, "telco.testr");             // default filenames expon180.1e6r
 strcpy(filein, "expon180.1e6r");  
strcpy(fileou, "telco.outc");              // ..

  j=0;                                       //counter( parameter number
    ...
    if (calc!=0) {
     ...
     // printf("riaz call type is not zero  and is %d\n" , calltype);
      ...
      //# decimal64ToNumber(&in64, &n);
       decimal64ToString(&in64, inputNumber);
       decDoubleFromString(&d_n, inputNumber, &set); // low call rate
